...
def update_market(model, threshold: float = EDGE_THRESHOLD, api_key: str | None = None) -> dict:
    ...
    for r in pend:
        ...
        if r["market_captured_at"] is None and lead is not None and 0 < lead <= ENTRY_MAX_HOURS:
            ...
            # 沒有賠率就不記注：記了也算不出損益，只會變成結算階段的地雷
            if (mkt_p_home is not None and model_p_home is not None
                    and m.get("dec_home") is not None and m.get("dec_away") is not None):
            ...
            if (mkt_line is not None and mkt_p_over is not None
                    and m.get("dec_over") is not None and m.get("dec_under") is not None):
...
def _settle_row(r) -> dict:
    ...
    整條排程死在第 5 步、儀表板根本沒機會產生。從 git 歷史撿回來的舊快照
    ...
    if r["paper_ml_side"]:
        ...
        if dec is not None and r["actual_winner"] is not None:
    ...
    if r["paper_total_side"] and r["market_total_line"] is not None:
        ...
        if actual_total is not None and dec is not None:
        ...
        clv, pts = _total_clv(side, line, r["closing_total_line"],
...
def settle_market(recompute: bool = True) -> int:
    ...
    for r in rows:
        try:
            ...
            bad += 1
            print(f"   ⚠ 結算 {r['game_id']} 時出錯，跳過（{type(e).__name__}: {e}）")
            continue
        if upd:
